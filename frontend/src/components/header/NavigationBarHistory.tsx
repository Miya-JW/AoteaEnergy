...
const NavigationBarHistory: React.FC<NavigationBarHistoryProps> = ({ page }) => {
    return (
        <div className='flex-left mt-100  '>
            ...
            <div className={`two-column-layout p-10-20 cursor-pointer ${page === 'generation-and-usage' ? 'background-menu' : ''}`} onClick={() => window.location.href = '/generation-and-usage'} >
                <div className='font-28'>Generation and Usage</div>
                <img src={ShareIcon} alt='share' className='ml-30' />
            </div>

            <div className={`two-column-layout p-10-20 ml-50 cursor-pointer ${page === 'energy-efficiency-ratio' ? 'background-menu' : ''}`} onClick={() => window.location.href = '/energy-efficiency-ratio'}>
                <div className='font-28 '>Energy Efficiency Ratio</div>
                <img src={ShareIcon} alt='share' className='ml-30' />
            </div>

            <div className={`two-column-layout p-10-20 ml-50 cursor-pointer ${page === 'generation-efficiency' ? 'background-menu' : ''}`} onClick={() => window.location.href = '/generation-efficiency'}>
